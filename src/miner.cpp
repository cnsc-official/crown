...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("CNSCMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("CNSCMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("CNSCMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cnsc-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("CNSCMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running CNSCMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadCNSCMiner() exception");
    } catch (...) {
        LogPrintf("ThreadCNSCMiner() exception");
    }

    LogPrintf("ThreadCNSCMiner exiting\n");
}
