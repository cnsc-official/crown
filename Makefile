...
pkgdatadir = $(datadir)/cnsc
pkgincludedir = $(includedir)/cnsc
pkglibdir = $(libdir)/cnsc
pkglibexecdir = $(libexecdir)/cnsc
...
CONFIG_HEADER = $(top_builddir)/src/config/cnsc-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/cnsc-config.h.in \
...
ACLOCAL = ${SHELL} /root/vps/code/cnsc/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /root/vps/code/cnsc/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /root/vps/code/cnsc/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /root/vps/code/cnsc/build-aux/missing automake-1.15
AWK = gawk
BDB_CPPFLAGS = 
...
LIBMEMENV = 
LIBOBJS = 
LIBS = -lcrypto -lanl -lcurl -lssl -lcrypto -lz
...
LUPDATE = 
MAINT = 
MAKEINFO = ${SHELL} /root/vps/code/cnsc/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
MINIUPNPC_LIBS = 
...
OTOOL64 = 
PACKAGE = cnsc
PACKAGE_BUGREPORT = http://bit.cnscs
PACKAGE_NAME = CNSC Core
PACKAGE_STRING = CNSC Core 1.0.0
PACKAGE_TARNAME = cnsc
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = 
abs_builddir = /root/vps/code/cnsc
abs_srcdir = /root/vps/code/cnsc
abs_top_builddir = /root/vps/code/cnsc
abs_top_srcdir = /root/vps/code/cnsc
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /root/vps/code/cnsc/build-aux/install-sh
...
SUBDIRS = src
GZIP_ENV = "-9n"
BITCOIND_BIN = $(top_builddir)/src/cnscd$(EXEEXT)
BITCOIN_QT_BIN = $(top_builddir)/src/qt/cnsc-qt$(EXEEXT)
BITCOIN_CLI_BIN = $(top_builddir)/src/cnsc-cli$(EXEEXT)
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_cnsc_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info \
  leveldb_baseline_filtered.info test_cnsc_coverage.info test_cnsc.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/CNSC-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/cnsc-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/cnsc-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/cnsc-config.h
$(top_srcdir)/src/config/cnsc-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/cnsc-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/CNSC-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/CNSC-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/CNSC-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_cnsc.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_cnsc -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_cnsc_filtered.info: test_cnsc.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_cnsc_filtered.info
#	$(MKDIR_P) qa/tmp
...
#test_cnsc_coverage.info: baseline_filtered_combined.info test_cnsc_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_cnsc_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_cnsc_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_cnsc_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_cnsc.coverage/.dirstamp:  test_cnsc_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_cnsc.coverage/.dirstamp total.coverage/.dirstamp

# CNSC: disable the outdated comparison tool test
...
clean-local:
	rm -rf test_cnsc.coverage/ total.coverage/ $(OSX_APP)
